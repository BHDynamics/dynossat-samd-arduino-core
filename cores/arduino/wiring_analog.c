...
uint32_t analogRead(uint32_t pin)
{
 ...
 //ATSAMR, for example, doesn't have a DAC
	  ...
	  #elif defined(PIN_DAC0) && !defined(PIN_DAC1)
	  if (pin == PIN_DAC0) {
	    uint8_t channel = 0;
	  #elif !defined(PIN_DAC0) && defined(PIN_DAC1) 
	  if (pin == PIN_DAC1) {
		uint8_t channel = 1;
	  #endif /* Pin DAC defines */
...
void analogWrite(uint32_t pin, uint32_t value)
{
 ...
 // ATSAMR, for example, doesn't have a DAC
	  ...
	  {
	    // DAC handling code
#if defined(__SAMD51__) && defined(PIN_DAC0) && defined(PIN_DAC1)
		if (pin == PIN_DAC0 || pin == PIN_DAC1) { // 2 DACs on A0 (PA02) and A1 (PA05)
			uint8_t channel = (pin == PIN_DAC0 ? 0 : 1);
#elif (defined(__SAMD51__) && defined(PIN_DAC0) && !defined(PIN_DAC1)) || !defined(__SAMD51__)
	    if (pin == PIN_DAC0) { // Only 1 DAC on A0 (PA02)
			uint8_t channel = 0;
#elif defined(__SAMD51__) && !defined(PIN_DAC0) && defined(PIN_DAC1)
        if (pin == PIN_DAC1) { // Only 1 DAC on A2 (PA05)
			uint8_t channel = 1;
#endif
